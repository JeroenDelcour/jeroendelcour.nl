...
module.exports = {
	...
	edit: function(article, callback) {
		db = new sqlite3.Database(dbfile);
		db.serialize(function() {
			var stmt = db.prepare("UPDATE articles SET title = ?, markdown = ?, html = ? WHERE rowid = ?;")
			stmt.run([article.title, article.markdown, marked(article.markdown), article.rowid], callback);
			stmt.finalize();
			db.close()
		});
	},
